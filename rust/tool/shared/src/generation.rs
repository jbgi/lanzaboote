...
impl Generation {
    ...
    pub fn version_tag(&self) -> String {
        format!("{}{}", self.label.clone().unwrap_or_else(|| self.version.to_string()), self.describe_specialisation(),)
    }
}

impl fmt::Display for Generation {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f, "{}", self.label.clone().unwrap_or_else(|| self.version.to_string()))
    }
}
